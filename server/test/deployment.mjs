...
const outputDir = await mkdtemp(resolve(tmpdir(), 'browser-mcp-deployment-'));
const packageArgs = [
...
try {
  ...
  const first = JSON.parse(await readFile(resolve(outputDir, 'artifacts.json'), 'utf8'));
  await run(process.execPath, packageArgs, { cwd: root });
  const second = JSON.parse(await readFile(resolve(outputDir, 'artifacts.json'), 'utf8'));
  ...
  const { stdout: doctorOutput } = await run(process.execPath, [
    ...
    '--artifacts', resolve(outputDir, 'artifacts.json'),
  ], { cwd: root });
